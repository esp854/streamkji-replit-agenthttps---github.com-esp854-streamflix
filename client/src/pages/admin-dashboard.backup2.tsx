...
function AdminDashboard() {
  ...
  const handleUpdateContent = (e: React.FormEvent) => {
    ...
    updateContentMutation.mutate({
      tmdbId: selectedContentForEdit.tmdbId,
      title,
      language,
      quality,
      mediaType,
  ...
  const updateContentMutation = useMutation({
    mutationFn: async (data: Partial<Content>) => {
      ...
      const response = await fetch(`/api/admin/content/${data.tmdbId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer " + token,
  ...
  // Mutation for adding video link
  const addVideoLinkMutation = useMutation({
    mutationFn: async (data: Partial<Content>) => {
      ...
      const response = await fetch("/api/admin/content/video-link", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer " + token,
  ...
  const deleteContentMutation = useMutation({
    mutationFn: async (tmdbId: string) => {
      ...
      const response = await fetch(`/api/admin/content/${tmdbId}`, {
        method: "DELETE",
        headers: {
          "Authorization": "Bearer " + token,
          "X-CSRF-Token": csrfToken,
  ...
  const handleDeleteContent = (content: Content) => {
    const confirmed = window.confirm(`Êtes-vous sûr de vouloir supprimer ${content.title}?`);
    if (confirmed) {
      deleteContentMutation.mutate(content.tmdbId);
    }
  };
  ...
  const handleCreateContent = () => {
    if (!selectedMovie) {
      toast({
        title: "Erreur",
        description: "Aucun film sélectionné.",
        variant: "destructive",
      });
      return;
    }
    ...
    createContentMutation.mutate({
      ...
      genres: selectedMovie.genres.map((genre: any) => genre.name).join(", "),
  ...
  // Handle searching content
  const handleSearchContent = async () => {
    setIsSearching(true);
    try {
      const response = await fetch(`/api/contents/tmdb?query=${searchQuery}`);
  ...
  const handleUpdateUser = (e: React.FormEvent) => {
    ...
    const admin = (form.querySelector('#admin') as HTMLInputElement)?.checked;
    const active = (form.querySelector('#active') as HTMLInputElement)?.checked;
    // Decode any HTML entities in the URL before sending to the server
    let cleanOdyseeUrl = odyseeUrl;
    if (odyseeUrl) {
...
import { Button, Label, Input, Switch, Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter, Select, SelectTrigger, SelectValue, SelectContent, SelectItem, toast } from "@/components/ui";
